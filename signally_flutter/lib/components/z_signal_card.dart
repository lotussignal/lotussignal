...
class _ZSignalCardState extends State<ZSignalCard> {
                  ...
                  children: [
                    Container(
                      decoration: BoxDecoration(
                          color: widget.signal.entryType == 'Long' ? appColorGreen : appColorRed, borderRadius: BorderRadius.circular(5)),
  ...
  _buildTargetCurrentPrice(bool isExpanded) {
    ...
    final entryVsCurrentPrice =
        widget.signal.compareEntryPriceWithCurrentPrice(price: appControlsProvider.getWSSymbolPrice(widget.signal), isPips: isForex);
    ...
    return ZCard(
      ...
      child: Row(
        children: [
          ...
          if (hasApiAccess)
            Text(appControlsProvider.getWSSymbolPrice(widget.signal).toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900)),
          Spacer(),
          if (hasApiAccess)
            Text(
              getPipsOrPercentStr(
